const features = [
  ...
  {
    title: 'Onde Atuamos',
    text: 'Porto e Norte de Portugal. Dispomos de equipas certificadas para responder em 48 horas em todo o território nacional.',
  },
]
      ...
      <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-3 gap-12">
        {features.map((f) => (
          <div key={f.title} className="border-l-2 border-yellow-400 pl-6">
            <h3 className="text-xl font-semibold mb-4 text-zinc-700">{f.title}</h3>
            <p className="text-zinc-700 leading-relaxed text-sm">{f.text}</p>
          </div>
        ))}
      </div>
    </section>
